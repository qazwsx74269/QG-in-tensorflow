...
class QGModel(object):
	def __init__(self,params):
		...
		self.para_pretrained_embedding = tf.convert_to_tensor(np.load(self.params.PARA_ID2EMBEDDING),dtype=tf.float32)
		self.build_model()
	...
	def build_model(self):
		...
		self.para_input = tf.placeholder(tf.int32, [None, None], name='encoder2_inputs')
		self.para_size = tf.placeholder(tf.int32, [None], name='encoder2_inputs_length')
		...
		para_emb = tf.nn.embedding_lookup(self.para_pretrained_embedding,self.para_input)
		...
		with tf.variable_scope("encoder_scope"):
			...
			with tf.variable_scope("encoder2"):
				self.enc_cell_fw2 = self._create_rnn_cell(self.params.PARA_HIDDEN_SIZE)
				self.enc_cell_bw2 = self._create_rnn_cell(self.params.PARA_HIDDEN_SIZE)
				enc_outputs2,enc_state2 = tf.nn.bidirectional_dynamic_rnn(
					self.enc_cell_fw2,self.enc_cell_bw2,para_emb,self.para_size,dtype=tf.float32)
				enc_outputs2 = tf.concat([enc_outputs2[0],enc_outputs2[1]],-1)
				# tuple_state2 = tuple([tf.contrib.rnn.LSTMStateTuple(tf.concat([enc_state2[0][idx].c,enc_state2[1][idx].c],-1),\
				# tf.concat([enc_state2[0][idx].h,enc_state2[1][idx].h],-1)) for idx in range(self.params.NUM_LAYERS)])
			tuple_state = tuple([tf.contrib.rnn.LSTMStateTuple(\
				tf.concat([enc_state[0][idx].c,enc_state[1][idx].c,enc_state2[0][idx].c,enc_state2[1][idx].c],-1),\
				tf.concat([enc_state[0][idx].h,enc_state[1][idx].h,enc_state2[0][idx].h,enc_state2[1][idx].h],-1))\
				 for idx in range(self.params.NUM_LAYERS)])
		...
		with tf.variable_scope("decoder_scope"):
			...
			self.dec_cell = self._create_rnn_cell((self.params.HIDDEN_SIZE+self.params.PARA_HIDDEN_SIZE)*2)
			self.attention_mechanism = tf.contrib.seq2seq.LuongAttention((self.params.HIDDEN_SIZE+self.params.PARA_HIDDEN_SIZE)*2,\
			...
			self.attention_cell = tf.contrib.seq2seq.AttentionWrapper(self.dec_cell,self.attention_mechanism,\
				attention_layer_size=(self.params.HIDDEN_SIZE+self.params.PARA_HIDDEN_SIZE)*2,alignment_history=True,\
