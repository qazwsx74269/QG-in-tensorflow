		...
		with tf.variable_scope("decoder_scope"):
			...
			if self.params.MODE in ['train','val']:
				...
				with tf.variable_scope("decoder"):
					...
					if self.params.BEAM_SEARCH:
						trg_ids = trg_ids.write(0,tf.fill([self.batch_size,self.params.BEAM_SIZE],self.params.SOS_ID))
					else:
						trg_ids = trg_ids.write(0,tf.fill([batch_size],self.params.SOS_ID))
					(finished, initial_inputs, initial_state) = inference_decoder.initialize()
					init_loop_var = (initial_state,trg_ids,0,initial_inputs)

					def continue_loop_condition(state,trg_ids,step,inputs):
					...
					def loop_body(state,trg_ids,step,inputs):
						...
						(next_outputs, next_state, next_inputs,decoder_finished) = \
							inference_decoder.step(step, inputs, state)
						# print(next_outputs.get_shape().as_list())
						# print(next_outputs)
						if self.params.BEAM_SEARCH:
							trg_ids = trg_ids.write(step+1,next_outputs.predicted_ids)
						else:
							trg_ids = trg_ids.write(step+1,next_outputs.sample_id)

						return next_state,trg_ids,step+1,next_inputs

					state,trg_ids,step,_= tf.while_loop(continue_loop_condition,loop_body,init_loop_var)
					self.alignment_history = state.cell_state.alignment_history.stack()

					self.decoder_predict_decode = trg_ids.stack()
