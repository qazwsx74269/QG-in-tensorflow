...
class Batcher(object):
    def __init__(self,params):
        ...
        with open(self.para,'r') as f:
            self.para_lines = f.readlines()
            # print(self.len)
            # print(self.len(self.para_lines))
            assert self.len == len(self.para_lines)
    ...
    def process(self):
        ...
        for (src_line,trg_line,para_line) in tqdm(zip(self.src_lines,self.trg_lines,self.para_lines)):
            src_line = src_line.strip().split()#字符串转为list
            src_line = [int(x) for x in src_line]#转为int
            para_line = para_line.strip().split()#字符串转为list
            para_line = [int(x) for x in para_line]#转为int        
            ...
            if len(src_line)<self.params.MAX_LEN_S and len(src_line)>1\
                ...
                and len(para_line)<self.params.MAX_LEN_P and len(para_line)>1:
                self.src_size.append(len(src_line))
                self.src_input.append(src_line)
                self.para_size.append(len(para_line))
                self.para_input.append(para_line)
    ...
    def get_batch(self):
        while True:
            ...
            for i in range(self.start,self.end):
                ...
                para_size_batch.append(self.para_size[index])

            max_src_size = max(src_size_batch)
            max_trg_size = max(trg_size_batch)
            max_para_size = max(para_size_batch)

            for i in range(self.start,self.end):
                index = self.indexs[i]
                s = self.src_input[index]
                q = self.trg_target[index]
                p = self.para_input[index]
                pad_s = [0]*(max_src_size-len(s))
                pad_q = [0]*(max_trg_size-len(q))
                pad_p = [0]*(max_para_size-len(p))
                src_input_batch.append(s+pad_s)
                para_input_batch.append(p+pad_p)
                trg_target_batch.append(q+pad_q)
                trg_input_batch.append([self.params.SOS_ID]+q[:-1]+pad_q)
            
            self.start += self.batch_size
            yield (len(src_input_batch),np.array(src_input_batch),np.array(src_size_batch),np.array(para_input_batch),\
                np.array(para_size_batch),np.array(trg_input_batch),np.array(trg_target_batch),np.array(trg_size_batch))
